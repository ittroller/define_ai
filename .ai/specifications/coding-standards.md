...
## 5. Backend (Python & Odoo)
- **Coding Style**: Tuân thủ PEP 8 cho Python code và Odoo Coding Guidelines.
- **Naming Convention**:
    - **Python**: `snake_case` cho hàm và biến, `PascalCase` cho class.
    - **Odoo Models**: Tên class nên phản ánh tên model (e.g., `class SaleOrder(models.Model)` cho `sale.order`).
    - **Odoo XML**: Sử dụng `snake_case` cho ID của các records.
- **Architecture (Odoo)**:
    - Phân tách rõ ràng giữa Logic (`models/`), Giao diện (`views/`), Dữ liệu (`data/`), và Security (`security/`).
    - Hạn chế viết logic phức tạp trực tiếp trong các phương thức của model; nên tách ra các Service hoặc Helper nếu cần.
- **Error Handling**:
    - Sử dụng `UserError` hoặc `ValidationError` từ `odoo.exceptions` để thông báo cho người dùng.
- **Performance**:
    - Luôn sử dụng `filtered()`, `mapped()`, `sorted()` thay vì vòng lặp for thủ công nếu có thể.
    - Tránh các câu truy vấn SQL thuần; ưu tiên Odoo ORM.
    - Sử dụng `prefetch` hiệu quả để tránh lỗi N+1.

### 5.1. Odoo Web Library (OWL) Standards
- **Component Design**: 
    - Chia nhỏ UI thành các OWL Components nhỏ, có tính tái sử dụng cao.
    - Mỗi Component nên có file `.js` và `.xml` (template) riêng biệt trong `static/src/`.
- **State Management**:
    - Sử dụng `useState` cho local state của component.
    - Đối với global state, sử dụng các service hoặc `reactive` store.
- **Hooks & Lifecycle**:
    - Sử dụng các hooks chuẩn của OWL như `onWillStart`, `onMounted`, `onWillUnmount`.
    - Đảm bảo dọn dẹp (cleanup) các event listeners trong `onWillUnmount`.
- **Naming Convention (OWL)**:
    - Component Class: PascalCase (e.g., `MyCustomComponent`).
    - Template Name: `module_name.ComponentName` (e.g., `web.MyCustomComponent`).
- **Assets Management**:
    - Đăng ký components/assets trong `__manifest__.py` thông qua key `assets` (Odoo v15+).
    - Phân chia assets theo bundles (e.g., `web.assets_backend`, `web.assets_frontend`).
