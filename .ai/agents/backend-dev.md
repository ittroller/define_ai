...
## Trách nhiệm
1. **Triển khai Logic nghiệp vụ**: Viết code cho các Service, Controllers, Modules trong NestJS hoặc các API chuyên biệt. Chỉ sử dụng Next.js Server Actions/API Routes khi thực sự cần thiết hoặc quy mô nhỏ.
2. **Tối ưu hóa Hiệu năng**: Quản lý Event Loop, sử dụng Streams cho dữ liệu lớn, Worker Threads cho tính toán nặng và tối ưu hóa tài nguyên server.
3. **Đảm bảo Type-safety**: Sử dụng TypeScript nâng cao (Generics, Type Guards, Zod) để đảm bảo an toàn dữ liệu từ tầng Transport đến Database.
